...
Figure~\ref{fig:vit_lr} shows the AdaDion V2 learning rate sweep on ViT-Small, where lr=0.005 is optimal.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/vit_lr_sweep.pdf}
\caption{AdaDion V2 learning rate sweep on ViT-Small.}
\label{fig:vit_lr}
\end{figure}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/communication.pdf}
...
\subsection{Compression vs Accuracy}

Figure~\ref{fig:rank_tradeoff} shows the relationship between rank fraction, validation accuracy, and communication compression for AdaDion V2 on ResNet-18 across five rank fractions. The optimal rank fraction is 0.5, which achieves 96.30\% accuracy with 1.8x compression over full all-reduce. Lower rank fractions provide higher compression at modest accuracy cost: rf=0.125 gives 4x compression with only 0.11\% accuracy loss (96.19\%). Full rank (rf=1.0) removes compression entirely and degrades accuracy to 95.99\%, 0.31\% below the optimal. This degradation at full rank suggests that the low-rank constraint acts as implicit regularization, preventing the optimizer from overfitting to gradient noise.

Figure~\ref{fig:compression} compares the communication compression ratio across all optimizers on both architectures. Dion and Dion2 achieve the highest compression (3.0--4.0x) due to their lower rank fraction (0.25). AdaDion V2 uses a higher rank fraction (0.5) that results in lower compression (1.5--1.8x) but higher accuracy. AdamW and Muon provide no compression because they all-reduce the full gradient.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/rank_tradeoff.pdf}
\caption{Rank fraction vs validation accuracy (left axis) and communication compression ratio for a $512 \times 4608$ layer (right axis). The optimal rank fraction (0.5) balances accuracy and compression.}
\label{fig:rank_tradeoff}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/compression_ratio.pdf}
\caption{Communication compression ratio relative to full gradient all-reduce on both architectures.}
...
\textbf{Small layers.} The final linear layer ($10 \times 512$) has $r_\text{cap} = \lfloor 0.7 \times 10 \rfloor = 7$. Since rank\_min $= 16 > r_\text{cap}$, the rank saturates at $r = 7$ from initialization and remains constant throughout training. The adaptive mechanism has no room to adjust.
...
Muon is 3.3x slower than AdamW on WRN-28-10 (20413s vs 6254s) while achieving 0.15\% lower accuracy, making it impractical at this scale without distributed communication savings to offset the compute cost. Figure~\ref{fig:wrn} shows the accuracy trend across widths.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/wrn_scaling.pdf}
\caption{Validation accuracy vs model width on WideResNet-28.}
\label{fig:wrn}
\end{figure}
...
AdaDion V2 achieves the best validation accuracy across all architectures and model scales tested: ResNet-18 (96.11\%), ViT-Small (89.66\%), and WideResNet-28-10 (96.91\%). The adaptive rank mechanism consistently outperforms Dion's fixed $d/4$ rank by 0.23--1.13\%, with the benefit persisting from 1.5M to 36.5M parameters. On 4$\times$A100 DDP, AdaDion V2 runs 3.6x faster than Muon while achieving higher accuracy, and reduces communication volume by 1.8x compared to full-gradient methods.

The rank-performance trade-off shows that rank fraction 0.5 balances accuracy and compression, while lower fractions provide up to 4x compression with modest accuracy loss. The width scaling experiment confirms that AdaDion is robust to configuration choices: the same hyperparameters work across a 24x range of model sizes without retuning.
